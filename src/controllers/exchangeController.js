...
const createExchange = async (req, res) => {
  try {
    const exchange = await exchangeService.createExchange(req.body, req.file);
...
const updateExchange = async (req, res) => {
  try {
    const exchange = await exchangeService.updateExchange(req.params.id, req.body, req.file);
    res.status(200).json(exchange);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
